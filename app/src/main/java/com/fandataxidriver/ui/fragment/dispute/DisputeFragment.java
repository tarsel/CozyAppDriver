...
public class DisputeFragment extends BaseBottomSheetDialogFragment implements DisputeIView {
    ...
    private void createDispute() {
        ...
        if (MvpApplication.DATUM_history != null) {
            ...
            map.put("request_id", MvpApplication.DATUM_history.getId());
            map.put("user_id", MvpApplication.DATUM_history.getUserId());
            map.put("dispute_type", "provider");
            map.put("provider_id", SharedHelper.getKey(Objects.requireNonNull(getContext()), AppConstant.SharedPref.USER_ID));
